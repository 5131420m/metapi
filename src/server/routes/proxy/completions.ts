      ...
      try {
        ...
        if (isStream) {
          ...
          if (streamResult.status === 'failed') {
            ...
            // Base budget only. A failed stream is reported as 502, which is never an
            ...
            if (canRetryChannelSelection(retryCount, forcedChannelId, retryBudget.baseMaxRetries)) {
              failureAccumulator.record(streamFailureAttempt);
              retryCount++;
              continue;
            }
            const terminal = failureAccumulator.resolveTerminal(streamFailureAttempt);
            return reply.code(terminal.status).send(terminal.payload);
          }
        ...
        if (failure) {
          ...
          // Base budget only. `detectProxyFailure` synthesizes 502 for a structurally
          // empty 2xx, which is never an indeterminate 4xx.
          if (detectedRetryable && canRetryChannelSelection(retryCount, forcedChannelId, retryBudget.baseMaxRetries)) {
            failureAccumulator.record(detectedFailureAttempt);
            retryCount += 1;
            continue;
          }
          ...
          const terminal = failureAccumulator.resolveTerminal(detectedFailureAttempt);
          return reply.code(terminal.status).send(terminal.payload);
        }
