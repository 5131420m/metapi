...
async function exportPreferencesSection(input?: {
  ...
  const settings = settingRows
    ...
    .filter((row) => input?.includeDownstreamErrorPolicy !== false || row.key !== 'downstream_error_policy')
    .map((row) => ({
      key: row.key,
      value: (() => {
  ...
  await db.transaction(async (tx: typeof db) => {
    ...
    for (const row of section.sites) {
      await tx.insert(schema.sites).values({
        ...
        customHeadersOverrideRequestHeaders: row.customHeadersOverrideRequestHeaders ?? true,
        ...
        forcedEndpoint: row.forcedEndpoint ?? null,
        codexIdentityMode: row.codexIdentityMode ?? 'off',
        apiEndpointSiteFallbackEnabled: row.apiEndpointSiteFallbackEnabled ?? true,
        apiEndpointSiteFallbackCooldownUntil: row.apiEndpointSiteFallbackCooldownUntil ?? null,
        apiEndpointSiteFallbackLastSelectedAt: row.apiEndpointSiteFallbackLastSelectedAt ?? null,
        apiEndpointSiteFallbackLastFailedAt: row.apiEndpointSiteFallbackLastFailedAt ?? null,
        apiEndpointSiteFallbackLastFailureReason: row.apiEndpointSiteFallbackLastFailureReason ?? null,
