 ...
 *
 * `signal` lets a caller abort an in-flight discovery scan (fork behavior).
 * `contextSourceScope` names the cache scope that discovered context lengths
 * are recorded under (upstream behavior). Both are optional and independent,
 * so they share one options bag instead of competing for the 4th parameter.
 */
...
export interface PlatformAdapter {
  ...
  getModels(baseUrl: string, token: string, platformUserId?: number, options?: GetModelsOptions): Promise<string[]>;
...
export abstract class BasePlatformAdapter implements PlatformAdapter {
  ...
  abstract getModels(baseUrl: string, token: string, platformUserId?: number, options?: GetModelsOptions): Promise<string[]>;
