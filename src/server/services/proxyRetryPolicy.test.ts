...
describe('proxyRetryPolicy', () => {
  ...
  it('leaves an unexplained 4xx terminal', () => {
    // Whether to spend one more channel on an unexplained 4xx is a per-downstream-key
    // decision owned by the surface failure toolkit. This predicate is stateless and
    // mocked with explicit factories by ~13 test files, so it must not answer that
    // question — it always reports the plain routing verdict.
    expect(shouldRetryProxyRequest(400, 'Upstream returned HTTP 400: rejected')).toBe(false);
    expect(shouldRetryProxyRequest(413, 'Upstream returned HTTP 413: too large')).toBe(false);
