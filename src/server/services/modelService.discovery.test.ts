...
describe('refreshModelsForAccount credential discovery', () => {
  ...
  it('preserves earlier context-length entries when later credential scans return only a subset', async () => {
    getApiTokenMock.mockResolvedValue(null);
    getModelsMock.mockImplementation(async (_baseUrl: string, token: string, _platformUserId: unknown, options?: { contextSourceScope?: string }) => {
      const contextScope = options?.contextSourceScope;
  ...
  it('uses the configured ai endpoint for direct model discovery credentials', async () => {
    ...
    expect(getModelsMock).toHaveBeenCalledWith(
      ...
      undefined,
      {
        ...
        contextSourceScope: expect.stringMatching(/^account:\d+:refresh:[^:]+:scan:\d+$/),
      },
    );
  });
