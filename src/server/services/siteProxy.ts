...
async function resolveSiteRequestConfigByRequestUrl(requestUrl: string): Promise<{
  ...
  if (!normalizedRequestUrl) {
    return { proxyUrl: null, customHeaders: null, customHeadersOverrideRequestHeaders: true };
  }
  ...
  return {
    ...
    customHeadersOverrideRequestHeaders: matchedRow?.customHeadersOverrideRequestHeaders !== false,
  };
}
...
function resolveSiteCustomHeadersMergePriority(
  ...
  // 未显式关闭时保持 fork 默认的“站点头优先”。
  return site?.customHeadersOverrideRequestHeaders === false ? 'request' : 'site';
}
