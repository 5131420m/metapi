...
const runtimeSettingsPayloadSchema = z.object({
  ...
  proxyDebugTargetModels: z.union([z.string(), z.array(z.string())]).optional(),
  ...
  proxyNonStreamTimeoutSec: nonNegativeNumber.optional(),
  proxyMediaTimeoutSec: nonNegativeNumber.optional(),
  timeoutCountsAsChannelFailure: z.boolean().optional(),
...
function formatSettingsPayloadError(error: z.ZodError): string {
  ...
  const runtimeMessages: Record<string, string> = {
    ...
    proxyDebugTargetModels: '代理调试目标模型格式无效：需要 string 或 string[]',
    ...
    proxyNonStreamTimeoutSec: '非流式响应超时必须是大于等于 0 的数字（秒）',
    proxyMediaTimeoutSec: '媒体生成响应超时必须是大于等于 0 的数字（秒）',
    timeoutCountsAsChannelFailure: '超时计入通道失败开关格式无效：需要 boolean',
