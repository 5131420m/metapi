...
describe('ensureSiteSchemaCompatibility', () => {
  it.each([
    {
      ...
      expectedSql: [
        ...
        'ALTER TABLE sites ADD COLUMN api_endpoint_site_fallback_enabled integer DEFAULT 1;',
        'UPDATE sites SET api_endpoint_site_fallback_enabled = 1 WHERE api_endpoint_site_fallback_enabled IS NULL;',
        'ALTER TABLE sites ADD COLUMN api_endpoint_site_fallback_cooldown_until text;',
        'ALTER TABLE sites ADD COLUMN api_endpoint_site_fallback_last_selected_at text;',
        'ALTER TABLE sites ADD COLUMN api_endpoint_site_fallback_last_failed_at text;',
        'ALTER TABLE sites ADD COLUMN api_endpoint_site_fallback_last_failure_reason text;',
        'CREATE TABLE IF NOT EXISTS site_api_endpoints (id integer PRIMARY KEY AUTOINCREMENT NOT NULL, site_id integer NOT NULL REFERENCES sites(id) ON DELETE cascade, url text NOT NULL, enabled integer DEFAULT 1, sort_order integer DEFAULT 0, cooldown_until text, consecutive_failure_count integer DEFAULT 0, failure_window_started_at text, last_failure_scope_id text, last_selected_at text, last_failed_at text, last_failure_reason text, created_at text DEFAULT (datetime(\'now\')), updated_at text DEFAULT (datetime(\'now\')));',
    ...
    },
    {
      ...
      expectedSql: [
        ...
        'UPDATE "sites" SET "global_weight" = 1 WHERE "global_weight" IS NULL OR "global_weight" <= 0',
        'ALTER TABLE "sites" ADD COLUMN "forced_endpoint" TEXT',
        'ALTER TABLE "sites" ADD COLUMN "api_endpoint_site_fallback_enabled" BOOLEAN DEFAULT TRUE',
        'UPDATE "sites" SET "api_endpoint_site_fallback_enabled" = TRUE WHERE "api_endpoint_site_fallback_enabled" IS NULL',
        'ALTER TABLE "sites" ADD COLUMN "api_endpoint_site_fallback_cooldown_until" TEXT',
        'ALTER TABLE "sites" ADD COLUMN "api_endpoint_site_fallback_last_selected_at" TEXT',
        'ALTER TABLE "sites" ADD COLUMN "api_endpoint_site_fallback_last_failed_at" TEXT',
        'ALTER TABLE "sites" ADD COLUMN "api_endpoint_site_fallback_last_failure_reason" TEXT',
        'CREATE TABLE IF NOT EXISTS "site_api_endpoints" ("id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "site_id" INTEGER NOT NULL REFERENCES "sites"("id") ON DELETE CASCADE, "url" TEXT NOT NULL, "enabled" BOOLEAN DEFAULT TRUE, "sort_order" INTEGER DEFAULT 0, "cooldown_until" TEXT, "consecutive_failure_count" INTEGER DEFAULT 0, "failure_window_started_at" TEXT, "last_failure_scope_id" TEXT, "last_selected_at" TEXT, "last_failed_at" TEXT, "last_failure_reason" TEXT, "created_at" TEXT DEFAULT to_char(timezone(\'UTC\', CURRENT_TIMESTAMP), \'YYYY-MM-DD HH24:MI:SS\'), "updated_at" TEXT DEFAULT to_char(timezone(\'UTC\', CURRENT_TIMESTAMP), \'YYYY-MM-DD HH24:MI:SS\'))',
        'CREATE UNIQUE INDEX IF NOT EXISTS "site_api_endpoints_site_url_unique" ON "site_api_endpoints" ("site_id", "url")',
        'CREATE INDEX IF NOT EXISTS "site_api_endpoints_site_enabled_sort_idx" ON "site_api_endpoints" ("site_id", "enabled", "sort_order")',
        'CREATE INDEX IF NOT EXISTS "site_api_endpoints_site_cooldown_idx" ON "site_api_endpoints" ("site_id", "cooldown_until")',
    ...
    },
    {
      ...
      expectedSql: [
        ...
        'UPDATE `sites` SET `global_weight` = 1 WHERE `global_weight` IS NULL OR `global_weight` <= 0',
        'ALTER TABLE `sites` ADD COLUMN `forced_endpoint` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `api_endpoint_site_fallback_enabled` BOOLEAN DEFAULT TRUE',
        'UPDATE `sites` SET `api_endpoint_site_fallback_enabled` = TRUE WHERE `api_endpoint_site_fallback_enabled` IS NULL',
        'ALTER TABLE `sites` ADD COLUMN `api_endpoint_site_fallback_cooldown_until` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `api_endpoint_site_fallback_last_selected_at` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `api_endpoint_site_fallback_last_failed_at` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `api_endpoint_site_fallback_last_failure_reason` TEXT NULL',
        'CREATE TABLE IF NOT EXISTS `site_api_endpoints` (`id` INT AUTO_INCREMENT PRIMARY KEY, `site_id` INT NOT NULL, `url` TEXT NOT NULL, `enabled` BOOLEAN DEFAULT TRUE, `sort_order` INT DEFAULT 0, `cooldown_until` VARCHAR(191) NULL, `consecutive_failure_count` INT DEFAULT 0, `failure_window_started_at` VARCHAR(191) NULL, `last_failure_scope_id` VARCHAR(191) NULL, `last_selected_at` VARCHAR(191) NULL, `last_failed_at` VARCHAR(191) NULL, `last_failure_reason` TEXT NULL, `created_at` VARCHAR(191) DEFAULT (DATE_FORMAT(NOW(), \'%Y-%m-%d %H:%i:%s\')), `updated_at` VARCHAR(191) DEFAULT (DATE_FORMAT(NOW(), \'%Y-%m-%d %H:%i:%s\')), CONSTRAINT `site_api_endpoints_site_fk` FOREIGN KEY (`site_id`) REFERENCES `sites`(`id`) ON DELETE CASCADE)',
  ...
  it('adds missing failure-streak columns to an existing endpoint table', async () => {
    const { inspector, executedSql } = createInspector('sqlite', {
      existingTables: ['site_api_endpoints', 'site_disabled_models'],
      existingColumns: [
        'proxy_url', 'use_system_proxy', 'custom_headers', 'external_checkin_url',
        'global_weight', 'forced_endpoint', 'api_endpoint_site_fallback_enabled',
        'api_endpoint_site_fallback_cooldown_until', 'api_endpoint_site_fallback_last_selected_at',
        'api_endpoint_site_fallback_last_failed_at', 'api_endpoint_site_fallback_last_failure_reason',
      ],
    });

    await ensureSiteSchemaCompatibility(inspector);

    expect(executedSql).toContain('ALTER TABLE site_api_endpoints ADD COLUMN consecutive_failure_count integer DEFAULT 0;');
    expect(executedSql).toContain('ALTER TABLE site_api_endpoints ADD COLUMN failure_window_started_at text;');
    expect(executedSql).toContain('ALTER TABLE site_api_endpoints ADD COLUMN last_failure_scope_id text;');
  });
});
