...
export type SiteTableCompatibilitySpec = {
  ...
  columns?: SiteColumnCompatibilitySpec[];
    ...
    columns: [
      {
        ...
        addSql: {
          sqlite: 'ALTER TABLE site_api_endpoints ADD COLUMN consecutive_failure_count integer DEFAULT 0;',
          mysql: 'ALTER TABLE `site_api_endpoints` ADD COLUMN `consecutive_failure_count` INT DEFAULT 0',
          postgres: 'ALTER TABLE "site_api_endpoints" ADD COLUMN "consecutive_failure_count" INTEGER DEFAULT 0',
        },

      },
      {
        column: 'failure_window_started_at',
        addSql: {
          sqlite: 'ALTER TABLE site_api_endpoints ADD COLUMN failure_window_started_at text;',
          mysql: 'ALTER TABLE `site_api_endpoints` ADD COLUMN `failure_window_started_at` VARCHAR(191) NULL',
          postgres: 'ALTER TABLE "site_api_endpoints" ADD COLUMN "failure_window_started_at" TEXT',
        },
      },
      {
        column: 'last_failure_scope_id',
        addSql: {
          sqlite: 'ALTER TABLE site_api_endpoints ADD COLUMN last_failure_scope_id text;',
          mysql: 'ALTER TABLE `site_api_endpoints` ADD COLUMN `last_failure_scope_id` VARCHAR(191) NULL',
          postgres: 'ALTER TABLE "site_api_endpoints" ADD COLUMN "last_failure_scope_id" TEXT',
        },
      },
    ],
    postCreateSql: {
  ...
  for (const spec of SITE_TABLE_COMPATIBILITY_SPECS) {
    ...
    if (!hasTable) {
      ...
      for (const columnSpec of spec.columns ?? []) {
        if (!await inspector.columnExists(spec.table, columnSpec.column)) {
          await executeAddColumn(inspector, columnSpec.addSql[inspector.dialect]);
        }
        if (columnSpec.normalizeSql) {
          await inspector.execute(columnSpec.normalizeSql[inspector.dialect]);
        }
      }
    }
